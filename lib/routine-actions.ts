...
import { generateRoutine, groupsFor, optionalLimit } from './engine';
...
export function swapChoices(log: RoutineLog, productId: string, products: Product[], history: RoutineLog[], at: Date, settings: RoutineSettings): SwapChoice[] {
 ...
 // Manual cleanser choices can cross time assignments in either direction.
 // Automatic routines keep each product's saved time assignment.
 const manualTiming=(batch: Product[])=>isCleanser(product) && batch.some(p=>p.timeOfDay!=='both' && p.timeOfDay!==log.timeOfDay);
 ...
 for (const p of products) {
  ...
  if(isCleanser(product)) {
   ...
   // A linked second cleanser is offered only inside its complete pair,
   // including when its oil is paused or otherwise unavailable.
   else if(!rolesFor(p).includes('first_cleanse') && !products.some(oil=>oil.pairWithId===p.id)) candidates.push([p]);
 ...
 return candidates.filter(batch=> {
  ...
  const cap=optionalLimit(log.timeOfDay,settings);
