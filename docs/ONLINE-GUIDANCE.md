...
Manual cleanser swaps support Round Lab ↔ the linked SKIN1004 oil/foam pair in both morning and evening routines. This overrides time assignments only for that choice, retaining availability, spacing, frequency and intensity checks. Automatic double cleansing remains evening-only.

September 11 follow-up: [Medicube Zero Pore Pad](https://medicube.us/products/zero-pore-pad-1) explicitly permits morning and evening use; [SKIN1004 Dark Spot Pad](https://www.skin1004.com/products/tone-brightening-dark-spot-ampoule-pad) gives after-cleansing instructions without an evening-only restriction. Pads are moved to evening once at the user's request, preserving notes, frequency and history; later product edits remain possible. This is a personal scheduling preference, not a claim that morning use is medically prohibited. Mist plus Centella ampoule was previously excluded by the morning optional-step cap, not by an ingredient compatibility finding. Mist-first mode now allows two optional steps within the configured limit.
