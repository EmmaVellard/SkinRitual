...
- Today with day browsing, Products, and editable History; no empty Rules navigation.
...
- Local date boundaries; automatic morning 05:00–16:59, evening otherwise; a manual override resets at the next period/date boundary.
- Native modal form with required fields, keyboard handling, and delete confirmation.
- Sixteen starter products, editable roles and schedules, duplicate-function selection, oil/foam pairing, rotation, intensity budgets, and saved explanations. See [DEFAULTS.md](../DEFAULTS.md).
- PAO/printed-date calculations with month-end clamping and 30-day notices.
- Versioned JSON backup/restore with validation, atomic replacement, and a one-level undo point.
- Eligible swaps and explicit skips, without modifying already used steps.
...
Data belongs to this browser and origin (scheme, hostname, port). Clearing site data or browser eviction can remove it. Safari and a Home Screen install may use separate storage. Backup/restore validates the versioned JSON before an atomic replacement and keeps a local undo point. Exported files omit that undo point; only the current products, logs, and settings are portable.
