...
export default function SettingsPage({settings,onSave,mutate,busy,canUndo}:{settings:RoutineSettings;onSave:(s:RoutineSettings)=>void;mutate:Mutate;busy:boolean;canUndo:boolean}) {
 ...
 <div className="settings-grid">
  ...
  <section className="settings-card"><h2>{tr('Routine preferences')}</h2>
   ...
   <Choices title={tr('Optional-step limit')} value={String(settings.maxOptionalSteps)} busy={busy} options={[0,1,2,3].map(n=>({value:String(n),label:String(n)}))} onChange={value=>onSave({...settings,maxOptionalSteps:Number(value)})}/>
   ...
   <p className="field-help">{tr('Morning allows one optional step, or up to two with mist first: mist followed by an ampoule. Your optional-step limit still applies. Preferences affect unstarted routines; they do not guarantee skin tolerance.')}</p>
  </section>
